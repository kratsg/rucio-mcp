...
| `--auth-type`           | —                         | from cfg    | Override RUCIO_AUTH_TYPE (stdio + shared-secret http; ignored in OIDC http)              |
| `--shared-secret`       | `RUCIO_MCP_SHARED_SECRET` | —           | Serve one pre-authenticated client behind a static bearer (http, single site; no OIDC)   |
