...
@pytest.fixture
def mcp_with_nomenclature() -> MCPServer:
    mcp = MCPServer("test")
...
@pytest.fixture
def mcp_without_nomenclature() -> MCPServer:
    mcp = MCPServer("test")
...
class TestNomenclatureResource:
    async def test_resource_uri_is_generic(
        self, mcp_with_nomenclature: MCPServer
    ) -> None:
    ...
    async def test_resource_not_registered_when_none(
        self, mcp_without_nomenclature: MCPServer
    ) -> None:
    ...
    async def test_resource_returns_nomenclature_content(
        ...
        result = await mcp_with_nomenclature.read_resource("rucio://nomenclature")
